...
# ═══════════════════════════════════════════════════════════════
#  CSS — pixel-faithful to Figma screenshots
# ═══════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
.stButton>button[kind="primary"] {
  background:#2B4EFF; color:white; border:none;
  border-radius:16px; font-size:1rem; padding:0.85rem;
}
...
.rp-mode  { font-size:.75rem; color:#9CA3AF; }
.rp-score { font-size:.95rem; font-weight:700; color:#22C55E; }
.rp-time  { font-size:.72rem; color:#9CA3AF; text-align:right; }

/* ── SETUP ── */
.sec-label { font-size:1rem; font-weight:700; color:#111827; margin:1rem 0 .55rem; }

.domain-card {
  background:white; border-radius:16px; padding:.95rem 1.1rem;
  margin-bottom:.65rem; display:flex; align-items:center; gap:.9rem;
  border:2px solid transparent; box-shadow:0 1px 6px rgba(0,0,0,.05);
}
...
.style-grid { display:grid; grid-template-columns:1fr 1fr; gap:11px; margin-bottom:1.1rem; }
.style-card {
  border-radius:16px; padding:.95rem; text-align:center;
  border:2px solid transparent; box-shadow:0 1px 6px rgba(0,0,0,.05);
}
...
.mic-circle {
  width:96px; height:96px; border-radius:50%; background:#3D4F6E;
  display:flex; align-items:center; justify-content:center; font-size:2.4rem;
  box-shadow:0 0 0 14px rgba(61,79,110,.1),0 0 0 28px rgba(61,79,110,.05);
}
...
.s-circle  {
  ...
  display:flex; align-items:center; justify-content:center;
  font-size:1.4rem; font-weight:700; color:white; float:right; margin-top:-6px;
}
...
.imp-card {
  ...
  margin-bottom:.85rem; display:flex; gap:11px; align-items:flex-start;
  box-shadow:0 1px 6px rgba(0,0,0,.05);
}
.imp-ico  { background:#F3F4F6; border-radius:10px; width:34px; height:34px;
...
.dw-card  {
  background:#F0FDF4; border-radius:16px; padding:1rem 1.1rem;
  margin-bottom:.85rem; border:1px solid #BBF7D0;
}
...
# ═══════════════════════════════════════════════════════════════
#  FREE AI — Groq (llama-3, 100% free at console.groq.com)
# ═══════════════════════════════════════════════════════════════
GROQ_URL = "https://api.groq.com/openai/v1/chat/completions"

def call_ai(messages, api_key, max_tokens=280):
...
# ═══════════════════════════════════════════════════════════════
def init():
    defs = dict(screen="home", field=None, mode="Friendly", current_q=0,
                history=[], scores=[], tips=[], strengths=[], answers=[],
                fb_ready=False, api_key="", q_cache={})
...
def get_followup(q_num):
    mode, field = st.session_state.mode, st.session_state.field
    sys = (f"You are a {'warm and encouraging' if mode=='Friendly' else 'strict and direct'} "
           f"interviewer for a {field} role. "
    ...
    msgs = [{"role":"system","content":sys}] + st.session_state.history + \
           [{"role":"user","content":f"Ask question {q_num} now."}]
    return call_ai(msgs, st.session_state.api_key, 120)

def evaluate(question, answer):
    mode, field = st.session_state.mode, st.session_state.field
    sys = (f"You evaluate a {field} interview answer. Style: {mode}. "
           "Return ONLY valid JSON, no markdown:\n"
           '{"score":<1.0-10.0>,"verdict":"<3-5 words>","improvement":"<1-2 sentence tip>","strengths":["<s1>","<s2>"]}')
    raw = call_ai([{"role":"system","content":sys},
                   {"role":"user","content":f"Question: {question}\nAnswer: {answer}"}],
                  st.session_state.api_key, 220)
...
# ═══════════════════════════════════════════════════════════════
def score_cls(s):
    if s >= 8:  return "s-green",  "c-green",  "#22C55E"
    if s >= 5:  return "s-orange", "c-orange", "#F59E0B"
...
if st.session_state.screen == "home":
    ...
    st.markdown('<div class="page-title">AI Interview Master</div>', unsafe_allow_html=True)
    st.markdown('<div class="page-sub">Your 24/7 Virtual Career Mentor</div>', unsafe_allow_html=True)

    # ── API KEY ──
    if not st.session_state.api_key:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown("**🔑 Enter your FREE Groq API Key**")
        st.caption("Get it free (no credit card) → [console.groq.com](https://console.groq.com)")
        ...
        <div class="card">
          ...
          <div style="display:flex;align-items:center;gap:10px;margin-top:4px">
            <div class="pb-bg" style="flex:1"><div class="pb-fill" style="width:81%"></div></div>
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown('<div class="card"><div class="stat-label">Interviews</div>'
                        '<div class="stat-val">24</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown('<div class="card"><div class="stat-label">Avg Score</div>'
        ...
        st.markdown(f'<div class="card"><div class="rp-head">Recent Practice</div>{rows}</div>',
                    unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="page-title" style="font-size:1.7rem">Setup Your Interview</div>',
                unsafe_allow_html=True)
    st.markdown('<div class="page-sub">Choose your domain and interviewer style</div>',
                unsafe_allow_html=True)
    ...
    for name, sub, icon, ic, bg in domains:
        ...
        st.markdown(
            ...
            f'<div style="flex:1"><div class="d-name">{name}</div>'
    ...
    st.markdown('<div class="sec-label">Choose Interviewer Style</div>', unsafe_allow_html=True)
    ...
    with cf:
        ...
        if st.button("Friendly", use_container_width=True, key="bf"):
            st.session_state.mode = "Friendly"; st.rerun()
    with cs:
    ...
    if st.button("Continue to Interview  →", use_container_width=True,
        ...
        st.session_state.update(screen="interview", current_q=0, history=[],
...
# ═══════════════════════════════════════════════════════════════
    ...
    if qi not in st.session_state.q_cache:
        with st.spinner("Preparing question..."):
            try:
                ...
                st.error(f"Could not load question: {e}"); st.stop()
    ...
    if not st.session_state.fb_ready:
        ans = st.text_area("ans", placeholder="Type your answer here or speak...",
    ...
    # ── FEEDBACK ──
        ...
        st.markdown(f'<div class="fb-qlabel">Question {qi+1} of 5</div>', unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            f'<div style="background:{bar_col};border-radius:8px;height:8px;width:{pct}%"></div></div>'
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="res-title">Session Complete! 🏆</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="res-sub">{st.session_state.field} · {st.session_state.mode} Mode · {TOTAL} Questions</div>',
                unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'<div style="background:#E5E7EB;border-radius:8px;height:8px;margin:12px auto;max-width:200px">'
        f'<div style="background:{vc};border-radius:8px;height:8px;width:{pct}%"></div></div>'
        f'<div style="font-size:.9rem;color:#6B7280;margin-top:6px">{verd}</div></div>',
        unsafe_allow_html=True)

    # Stats row
    c1, c2, c3 = st.columns(3)
    for col, lbl, val in [(c1,"Answered",len(valid)),(c2,"Avg Score",avg),(c3,"Best Score",best)]:
        with col:
            st.markdown(
    ...
    for i in range(len(st.session_state.scores)):
        ...
        with st.expander(f"Q{i+1}: {q[:55]}… — {lbl}"):
            ...
            if tip and "Skipped" not in tip:
                st.markdown(
                    f'<div class="dw-card" style="background:#FFFBEB;border-color:#FDE68A">'
                    f'<div style="color:#92400E;font-size:.83rem">💡 {tip}</div></div>',
                    unsafe_allow_html=True)

    st.markdown("<div style='margin-top:.8rem'></div>", unsafe_allow_html=True)
    if st.button("🔄  Practice Again", use_container_width=True, type="primary"):
        key_save = st.session_state.api_key
        for k in list(st.session_state.keys()):
            del st.session_state[k]
        init()
        st.session_state.api_key = key_save
        st.session_state.screen  = "setup"
        st.rerun()
