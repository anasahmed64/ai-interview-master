...
.hero-title {
  font-size: 2.1rem; font-weight: 800; color: white; line-height: 1.2;
...
.rp-head { font-size: 0.95rem; font-weight: 700; color: white; margin-bottom: 0.8rem; }
.rp-row {
  display: flex; justify-content: space-between; align-items: center;
  padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.07);
}
.rp-name { font-size: 0.88rem; font-weight: 600; color: rgba(255,255,255,0.9); }
.rp-mode { font-size: 0.72rem; color: rgba(255,255,255,0.4); margin-top: 2px; }
.rp-score { font-size: 1rem; font-weight: 700; color: #4ade80; }
.rp-time { font-size: 0.7rem; color: rgba(255,255,255,0.35); text-align: right; }

/* ── SETUP ── */
.sec-lbl {
  font-size: 0.75rem; font-weight: 700; color: rgba(255,255,255,0.45);
  letter-spacing: 0.1em; text-transform: uppercase; margin: 1.2rem 0 0.7rem;
}
.domain-card {
  background: rgba(255,255,255,0.06); border: 1.5px solid rgba(255,255,255,0.1);
  border-radius: 20px; padding: 1rem 1.1rem; margin-bottom: 0.6rem;
...
.d-name { font-size: 0.97rem; font-weight: 700; color: white; }
.d-sub { font-size: 0.77rem; color: rgba(255,255,255,0.45); margin-top: 2px; }
.d-check { font-size: 1.1rem; margin-left: auto; }

.mode-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 1rem; }
.mode-card {
...
.mode-card.sel-f { background: rgba(74,222,128,0.1); border-color: rgba(74,222,128,0.5); box-shadow: 0 0 0 3px rgba(74,222,128,0.1); }
.mode-card.sel-s { background: rgba(239,68,68,0.1); border-color: rgba(239,68,68,0.5); box-shadow: 0 0 0 3px rgba(239,68,68,0.1); }
.mode-icon { font-size: 2rem; margin-bottom: 6px; }
.mode-lbl { font-size: 0.9rem; font-weight: 700; color: white; }
.mode-desc { font-size: 0.72rem; color: rgba(255,255,255,0.45); margin-top: 3px; }
...
.mic-outer {
  ...
  background: rgba(102,126,234,0.1); border: 2px solid rgba(102,126,234,0.2);
...
.mic-inner {
  ...
  background: linear-gradient(135deg, #667eea, #764ba2);
  display: flex; align-items: center; justify-content: center;
  font-size: 2.2rem; box-shadow: 0 8px 32px rgba(102,126,234,0.6);
}
.mic-lbl { font-size: 0.82rem; color: rgba(255,255,255,0.4); margin-top: 10px; }

.q-card {
  background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.12);
  border-radius: 20px; padding: 1.2rem 1.3rem; margin-bottom: 1rem;
}
.q-lbl { font-size: 0.72rem; font-weight: 600; color: rgba(102,126,234,0.8); margin-bottom: 6px; letter-spacing: 0.08em; text-transform: uppercase; }
...
.imp-card {
  ...
  border-radius: 18px; padding: 1rem 1.1rem; margin-bottom: 0.85rem;
...
.imp-txt { font-size: 0.82rem; color: rgba(255,255,255,0.65); line-height: 1.55; }
...
.dw-item { font-size: 0.82rem; color: rgba(255,255,255,0.65); margin-bottom: 4px; display: flex; align-items: center; gap: 6px; }
...
.ov-verdict { font-size: 0.9rem; color: rgba(255,255,255,0.6); margin-top: 8px; }
</style>
...
if st.session_state.screen == "home":
    ...
    st.markdown('<div class="hero-badge">🎙 AI-Powered • 24/7 Available</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-title">Master Your<br><span>Interview Skills</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-sub">Practice with AI, get instant feedback, land your dream job.</div>', unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown('<div class="stat-box"><div class="stat-num">24</div><div class="stat-lbl">Total Sessions</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="stat-box"><div class="stat-num">8.2</div><div class="stat-lbl">Avg Score /10</div></div>', unsafe_allow_html=True)

    st.markdown("<div style='margin-top:1rem'></div>", unsafe_allow_html=True)

    rows = "".join([
        f'<div class="rp-row">'
        f'<div><div class="rp-name">{p["name"]}</div><div class="rp-mode">{p["mode"]}</div></div>'
        f'<div style="text-align:right"><div class="rp-score">{p["score"]}</div><div class="rp-time">{p["time"]}</div></div>'
        f'</div>'
        for p in RECENT
    ])
    st.markdown(f'<div class="gcard"><div class="rp-head">Recent Practice</div>{rows}</div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="font-size:1.6rem;font-weight:800;color:white;margin-bottom:.3rem">Setup Interview</div>', unsafe_allow_html=True)
    st.markdown('<div style="font-size:.85rem;color:rgba(255,255,255,.45);margin-bottom:.5rem">Choose your domain and interviewer style</div>', unsafe_allow_html=True)

    st.markdown('<div class="sec-lbl">Select Domain</div>', unsafe_allow_html=True)

    domains = [
        ("Software Engineering", "Technical & coding interviews",  "💻", "linear-gradient(135deg,#667eea,#764ba2)"),
        ("Human Resources",      "Behavioral & HR interviews",     "👤", "linear-gradient(135deg,#f093fb,#f5576c)"),
        ("Marketing",            "Strategy & creative interviews", "🎯", "linear-gradient(135deg,#4facfe,#00f2fe)"),
    ]

    for name, sub, icon, grad in domains:
        sel = st.session_state.field == name
        check = "✅" if sel else ""
        st.markdown(
            f'<div class="domain-card {"sel" if sel else ""}">'
            f'<div class="d-icon-wrap" style="background:{grad}">{icon}</div>'
            f'<div style="flex:1"><div class="d-name">{name}</div><div class="d-sub">{sub}</div></div>'
            f'<div class="d-check">{check}</div></div>', unsafe_allow_html=True)
        if st.button(name, key=f"d_{name}", use_container_width=True):
    ...
    with cf:
        ...
        st.markdown(
            f'<div class="mode-card {"sel-f" if sf else ""}">'
            '<div class="mode-icon">🙂</div>'
            '<div class="mode-lbl">Friendly</div>'
            '<div class="mode-desc">Encouraging & supportive</div></div>', unsafe_allow_html=True)
    ...
    with cs:
        ...
        st.markdown(
            f'<div class="mode-card {"sel-s" if ss else ""}">'
            '<div class="mode-icon">🎯</div>'
            '<div class="mode-lbl">Strict</div>'
            '<div class="mode-desc">Direct & challenging</div></div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f'<div class="int-mode">{"🙂" if mode=="Friendly" else "🎯"} {mode} Mode</div></div>'
        f'<div class="q-pill">Q {qi+1} / {TOTAL}</div></div>', unsafe_allow_html=True)

    # Mic
    st.markdown(
        '<div class="mic-wrap"><div class="mic-outer"><div class="mic-inner">🎙️</div></div>'
        '<div class="mic-lbl">AI is listening...</div></div>', unsafe_allow_html=True)
    ...
    if not st.session_state.fb_ready:
        ...
        st.markdown(
            ...
            f'<div class="sc-top">'
            f'<div><div class="sc-label">YOUR SCORE</div>'
            f'<span class="sc-num {nc}">{sc}</span><span class="sc-den">/10</span></div>'
            f'<div class="sc-icon {cc}">↗</div></div>'
            f'<div class="sc-bar-bg"><div style="background:{vc};border-radius:6px;height:6px;width:{pct2}%"></div></div>'
            f'<div class="sc-verdict" style="color:{vc}">✓ {verd}</div></div>',
            unsafe_allow_html=True)

        if tip:
            st.markdown(
                f'<div class="imp-card"><div class="imp-ico">⚡</div>'
                f'<div><div class="imp-ttl">Key Improvement</div>'
                f'<div class="imp-txt">{tip}</div></div></div>', unsafe_allow_html=True)

        if strs:
            items = "".join([f'<div class="dw-item">✅ {s}</div>' for s in strs])
            st.markdown(f'<div class="dw-card"><div class="dw-ttl">What You Did Well</div>{items}</div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f'<div class="res-sub">{st.session_state.field} · {st.session_state.mode} Mode</div></div>',
        unsafe_allow_html=True)

    st.markdown(
        f'<div class="overall-card">'
        f'<div style="font-size:.75rem;color:rgba(255,255,255,.45);letter-spacing:.1em;text-transform:uppercase;margin-bottom:8px">Overall Score</div>'
        f'<span class="ov-num">{avg}</span><span class="ov-den">/10</span>'
        f'<div style="background:rgba(255,255,255,.1);border-radius:6px;height:6px;margin:14px auto;max-width:180px">'
        f'<div style="background:{vc};border-radius:6px;height:6px;width:{pct}%"></div></div>'
        f'<div class="ov-verdict">{verd}</div></div>',
        unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    for col, lbl, val in [(c1,"Answered",len(valid)),(c2,"Average",avg),(c3,"Best",best)]:
        with col:
            st.markdown(f'<div class="stat-box"><div class="stat-num">{val}</div><div class="stat-lbl">{lbl}</div></div>', unsafe_allow_html=True)

    st.markdown('<div style="font-size:.75rem;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:.1em;text-transform:uppercase;margin:1.2rem 0 .7rem">Question Breakdown</div>', unsafe_allow_html=True)
    ...
    for i in range(len(st.session_state.scores)):
        ...
        with st.expander(f"Q{i+1}: {q[:50]}… — {lbl}"):
            ...
            if tip and "Skipped" not in tip:
                st.markdown(f'<div class="imp-card"><div class="imp-ico">💡</div><div><div class="imp-ttl">Tip</div><div class="imp-txt">{tip}</div></div></div>', unsafe_allow_html=True)
