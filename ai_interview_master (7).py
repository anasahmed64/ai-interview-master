...
# ── AI via Claude API (Anthropic) ──────────────────────────────
API_KEY = st.secrets["API_KEY"] if "API_KEY" in st.secrets else ""
API_URL = "https://api.anthropic.com/v1/messages"

def call_ai(system_prompt, user_message, max_tokens=300):
    headers = {
        "x-api-key": API_KEY,
...
def get_followup(q_num):
    ...
    sys_p = (f"You are a {tone} interviewer for a {field} role. "
             ...
             "Based on the conversation history, ask ONE smart follow-up interview question. "
             "Return ONLY the question. No numbering, no preamble.")
    user_msg = f"Conversation so far:\n{history_text}\n\nAsk question {q_num} now."
    return call_ai(sys_p, user_msg, 120)


def evaluate(question, answer):
    mode  = st.session_state.mode
    field = st.session_state.field
    sys_p = (f"You evaluate a {field} interview answer. Interviewer style: {mode}. "
             "Return ONLY valid JSON with no markdown:\n"
             '{"score": <number 1.0 to 10.0>, "verdict": "<3-5 word phrase>", '
             '"improvement": "<one specific tip in 1-2 sentences>", '
             '"strengths": ["<strength 1>", "<strength 2>"]}')
    user_msg = f"Question: {question}\nAnswer: {answer}"
    raw = call_ai(sys_p, user_msg, 250)
...
if st.session_state.screen == "home":
    ...
    rows = "".join([
        f'<div class="rp-row"><div><div class="rp-name">{p["name"]}</div>'
...
# ── SCREEN: INTERVIEW ──────────────────────────────────────────
    ...
    st.markdown(
        '<div class="mic-wrap"><div class="mic-circle">🎙️</div>'
    ...
    st.markdown(
        f'<div class="card"><div class="q-lbl">Question {qi + 1}</div>'
    ...
    if not st.session_state.fb_ready:
        ...
        if strs:
            ...
            st.markdown(
                f'<div class="dw-card"><div class="dw-ttl">What You Did Well</div>'
...
# ── SCREEN: RESULTS ────────────────────────────────────────────
    ...
    st.markdown(
        ...
        f'{st.session_state.mode} Mode · {TOTAL} Questions</div>', unsafe_allow_html=True)

    st.markdown(
        f'<div class="card" style="text-align:center;padding:1.4rem">'
        f'<div style="font-size:.82rem;color:#9CA3AF">Overall Score</div>'
        f'<span class="score-num {nc}">{avg}</span><span class="score-den">/10</span>'
