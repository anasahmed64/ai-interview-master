...
/* ── PREMIUM BUTTON CARDS (Fixes Dual Buttons) ── */
.stButton > button {
...
/* Primary Button Style (Selected/Action) */
...
/* Secondary Button Style (Unselected/Cards) */
...
/* ── TEXT AREA (Contrast Fix) ── */
.stTextArea textarea {
  border-radius: 18px !important;
  border: 1.5px solid rgba(255,255,255,0.2) !important;
  font-family: 'Plus Jakarta Sans', sans-serif !important;
  font-size: 0.95rem !important;
  background: rgba(0, 0, 0, 0.3) !important; /* Darkened for pure white text visibility */
...
/* ── EXPANDERS (Results Breakdown Contrast Fix) ── */
...
.stat-box { background: rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.1); border-radius: 18px; padding: 1rem; text-align: center; }
...
# Randomly pull 5 questions per session so the user isn't answering 50 at once!
TOTAL = 5 
...
# ════════════════════════════════════════════════════════════════
#  SCREEN: SETUP (Fixed Dual Buttons!)
# ════════════════════════════════════════════════════════════════
    ...
    for name, sub, icon in domains:
        ...
        # This replaces the custom HTML with a stylized native Streamlit button
    ...
    # Render Mode Buttons dynamically
    cf, cs = st.columns(2)
    with cf:
        is_friendly = (st.session_state.mode == "Friendly")
        if st.button("🙂   **Friendly** \n\nEncouraging", key="bf", type="primary" if is_friendly else "secondary", use_container_width=True):
            st.session_state.mode = "Friendly"
            st.rerun()
    with cs:
        is_strict = (st.session_state.mode == "Strict")
        if st.button("🎯   **Strict** \n\nDirect", key="bs", type="primary" if is_strict else "secondary", use_container_width=True):
...
# ════════════════════════════════════════════════════════════════
    ...
    for i in range(len(st.session_state.scores)):
        ...
        # Streamlit Native Expander completely fixed with CSS injections above
