...
html, body, [class*="css"] { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #0F0C29 !important; }
...
/* ── STRICT FIX FOR WHITE BOX / TEXT AREA ── */
.stTextArea textarea {
...
.stTextArea textarea::placeholder { color: rgba(255,255,255,0.3) !important; -webkit-text-fill-color: rgba(255,255,255,0.3) !important; }

/* ── EXPANDERS (Results Page) ── */
...
/* ── RECENT PRACTICE REDESIGN ── */
.gcard { background: rgba(20, 20, 35, 0.5); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.08); border-radius: 24px; padding: 1.5rem; margin-bottom: 1.5rem; }
.rp-head { font-size: 1.2rem; font-weight: 800; color: #ffffff; margin-bottom: 1.2rem; letter-spacing: 0.02em; }
.rp-row { display: flex; justify-content: space-between; align-items: center; padding: 1rem; background: rgba(255,255,255,0.03); border-radius: 16px; margin-bottom: 0.5rem; border: 1px solid rgba(255,255,255,0.05); }
...
.stat-box { flex: 1; background: rgba(20,20,35,0.5); border: 1px solid rgba(255,255,255,0.08); border-radius: 18px; padding: 1.2rem 0.5rem; text-align: center; }
.stat-num { font-size: 1.8rem; font-weight: 800; color: white; }
.stat-lbl { font-size: 0.75rem; color: rgba(255,255,255,0.45); margin-top: 4px; text-transform: uppercase; letter-spacing: 0.05em; }

.sec-lbl { font-size: 0.8rem; font-weight: 700; color: rgba(255,255,255,0.45); letter-spacing: 0.1em; text-transform: uppercase; margin: 1.5rem 0 0.8rem; }
...
.q-card { background: rgba(20,20,35,0.6); border: 1px solid rgba(102,126,234,0.3); border-radius: 20px; padding: 1.5rem; margin-bottom: 1.5rem; }
.q-lbl { font-size: 0.75rem; font-weight: 700; color: #a5b4fc; margin-bottom: 8px; letter-spacing: 0.08em; text-transform: uppercase; }
.q-text { font-size: 1.1rem; font-weight: 600; color: white; line-height: 1.6; }
...
.score-card { background: rgba(20,20,35,0.5); border: 1px solid rgba(255,255,255,0.12); border-radius: 22px; padding: 1.5rem; margin-bottom: 1rem; }
.sc-top { display: flex; justify-content: space-between; align-items: flex-start; }
.sc-label { font-size: 0.75rem; color: rgba(255,255,255,0.45); margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.05em; }
...
.imp-card { background: rgba(251,146,60,0.1); border: 1px solid rgba(251,146,60,0.3); border-radius: 18px; padding: 1rem 1.1rem; margin-bottom: 0.85rem; display: flex; gap: 12px; align-items: flex-start; }
.imp-ico { background: rgba(251,146,60,0.2); border-radius: 10px; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; font-size: 1rem; flex-shrink: 0; }
.imp-ttl { font-size: 0.9rem; font-weight: 700; color: #fb923c; margin-bottom: 4px; }
.imp-txt { font-size: 0.85rem; color: rgba(255,255,255,0.9); line-height: 1.55; }
.dw-card { background: rgba(74,222,128,0.08); border: 1px solid rgba(74,222,128,0.3); border-radius: 18px; padding: 1rem 1.1rem; margin-bottom: 0.85rem; }
.dw-ttl { font-size: 0.9rem; font-weight: 700; color: #4ade80; margin-bottom: 8px; }
.dw-item { font-size: 0.85rem; color: rgba(255,255,255,0.9); margin-bottom: 4px; display: flex; align-items: center; gap: 6px; }
...
.overall-card { background: linear-gradient(135deg, rgba(102,126,234,0.15), rgba(118,75,162,0.15)); border: 1px solid rgba(102,126,234,0.4); border-radius: 24px; padding: 1.5rem; text-align: center; margin: 1rem 0; }
.ov-num { font-size: 4rem; font-weight: 800; color: white; line-height: 1; }
.ov-den { font-size: 1.5rem; color: rgba(255,255,255,0.4); }
.ov-verdict { font-size: 1rem; font-weight: 600; color: #a5b4fc; margin-top: 10px; }
</style>
...
# CHANGE: SET TOTAL TO 50 SO IT USES ALL 50 QUESTIONS
TOTAL = 50 
...
def get_questions(field):
    ...
    # Returns 50 questions shuffled
    return random.sample(qs, min(TOTAL, len(qs)))

def get_followup(q_num):
    field = st.session_state.field
    sys_p = (f"You are an interviewer for a {field} role. "
...
if st.session_state.screen == "home":
    ...
    <div class="gcard">
      <div class="score-hero">
        <div>
          <div style="font-size:.75rem;color:rgba(255,255,255,.45);margin-bottom:4px;letter-spacing:0.05em;">CAREER READINESS</div>
      ...
      <div style="display:flex;justify-content:space-between;margin-top:8px">
        <span style="font-size:.75rem;color:rgba(255,255,255,.4)">Beginner</span>
    ...
    rows = "".join([
        f'<div class="rp-row"><div><div class="rp-name">{p["name"]}</div><div class="rp-mode">{p["mode"]}</div></div><div><div class="rp-score">{p["score"]}</div><div class="rp-time">{p["time"]}</div></div></div>'
        for p in RECENT
    ])
    
    st.markdown(f'<div class="gcard" style="padding: 1.2rem;"><div class="rp-head">Recent Practice</div>{rows}</div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.fb_ready:
        ans = st.text_area("ans", placeholder="Type your answer here...",
                           height=130, label_visibility="collapsed", key=f"a{qi}")
        ...
        st.markdown(f'<div class="sc-label" style="margin-bottom:10px">Feedback for Q{qi+1}</div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    for i in range(len(st.session_state.scores)):
        ...
        with st.expander(f"Q{i+1}: {q[:45]}… — {lbl}"):
            # Replaced markdown block with plain HTML to guarantee no "code box" formatting
            st.markdown(f'<div style="color:white; font-size: 0.95rem; margin-bottom: 15px;"><b>Your Answer:</b><br>{st.session_state.answers[i]}</div>', unsafe_allow_html=True)
