...
/* ── TEXT AREA (Strict Dark Theme & White Text Fix) ── */
div[data-baseweb="textarea"] {
    background-color: rgba(0, 0, 0, 0.3) !important;
    border: 1.5px solid rgba(255, 255, 255, 0.2) !important;
    border-radius: 16px !important;
}
div[data-baseweb="textarea"] > div { background-color: transparent !important; }
...
/* ── RECENT PRACTICE SECTION (Visibility & Alignment Fix) ── */
...
# Changed to ALL 50 questions per session based on your request!
TOTAL = 50
...
def get_questions(field):
    ...
    # Will shuffle so the 50 questions are in a fresh order every time
...
if st.session_state.screen == "home":
    ...
    rows = "".join([
        ...
        f'<div><div class="rp-score">{p["score"]}</div><div class="rp-time">{p["time"]}</div></div>'
        f'</div>'
        for p in RECENT
    ])
